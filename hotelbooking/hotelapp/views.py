...
def search(request):
    ...
    #hotels=Hotel.objects.filter(name__exact=name)
   # hotels=Hotel.objects.filter(id=name)
    hotels=Hotel.objects.filter(name__startswith=name)
    return render(request,'listHotels.html', {'hotels': hotels})
...
def booking_table(request):
    if request.method == 'POST':
        ...
        c_date1=str(c_date).replace("-","")
        OrderNo=c_date1+str(booking_table.id)
        booking_table.ordernumber=OrderNo
        booking_table.save()
        emailId=request.session["username"]
        customer1=Customer.objects.get(email=emailId)
        totalprice=0
        booking=Booking.objects.filter(customer=customer1)
        for price in   booking:
        ...
        return render(request, 'Payment.html',{"data":booking_table,"tprice":amount,"context":context})
...
def paymentsuccess(request):
    ...
    #tbill = request.get("total_bill")
    order_no = request.GET.get("ordernumber")
    ...
    send_mail(subject=subject, message = email_body, from_email = from_email, recipient_list = [email]) 
